...
namespace Microsoft.Samples.Kinect.KinectFusionHeadScanning
{
    ...
    public static class Helper
    {
        ...
        public static Size GetImageSize(ColorImageFormat imageFormat)
        {
            switch (imageFormat)
            {
        ...
        /// <returns>Returns the distance as float value</returns>
        public static float Distance(Vector3 vectorA, Vector3 vectorB)
        {
            Vector3 diff = new Vector3();
            diff.X = Math.Abs(vectorA.X - vectorB.X);
            diff.Y = Math.Abs(vectorA.Y - vectorB.Y);
            diff.Z = Math.Abs(vectorA.Z - vectorB.Z);
            return Length(diff);
        }
